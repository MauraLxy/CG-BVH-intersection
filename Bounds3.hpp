                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    // Calculate t values for intersections with each pair of slab planes
    ...
    // Check whether direction is negative
    if(dirIsNeg[0]==0){
    ...
    // Get the enter and exit t
    double tEnter = std::max({txMin,tyMin,tzMin});
    double tExit = std::min({txMax,tyMax,tzMax});
