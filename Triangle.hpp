...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    const Vector3f& s0 = ray.origin - v0;
    const Vector3f& s1 = crossProduct(ray.direction, e2);
    const Vector3f& s2 = crossProduct(s0, e1);

    float t = dotProduct(s2,e2)/dotProduct(s1,e1);
    float b1 = dotProduct(s1,s0)/dotProduct(s1,e1);
    float b2 = dotProduct(s2,ray.direction)/dotProduct(s1,e1);
    
    if ( t>= 0.f && b1 >= 0.f && b2 >=0.f && (1.f-b1-b2) >= 0.f)
    {
        intersected = true;
        dist = t;
    }
